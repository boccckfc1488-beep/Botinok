...
class Storage:
    def __init__(self):
        ...
        self.subscription_channel: Optional[str] = None  # "@channel" или "-100..."
    ...
    def load(self):
        ...
        try:
            ...
            for k, v in data.get("users", {}).items():
                u = User(**v)
                self.users[int(k)] = u
            log.info(f"Loaded {len(self.users)} users, {len(self.added_chats)} chats, channel={self.subscription_channel}")
        except Exception:
...
async def check_subscription(user_id: int) -> bool:
    """Проверяет подписку на канал. Если канал не задан — считаем подписанным."""
...
async def issue_card(chat_id: int, u: User, is_daily: bool = True):
    ...
    if sub and not u.subscription_bonus_claimed and store.subscription_channel:
        bonus = 1
...
async def handle_message(msg: dict):
    ...
    if text.startswith("/start") or text.startswith("/menu"):
        await bot.send_message(chat_id,
            ...
            + (f"Подпишись на {store.subscription_channel} — и получишь +1 карту!\n\n" if store.subscription_channel else "\n")
            + "Выбери действие:",
            reply_markup=main_menu_kb(owner=is_owner(u)))
        return
    ...
    if is_owner(u) and text.startswith("/setchannel"):
        parts = text.split(maxsplit=1)
        if len(parts) >= 2:
            store.subscription_channel = parts[1].strip()
            store.save()
            await bot.send_message(chat_id, f"📡 Канал подписки: {store.subscription_channel}")
        else:
            await bot.send_message(chat_id, "❌ /setchannel @channel")
        return
...
async def handle_state(user_id: int, chat_id: int, text: str, u: User) -> bool:
    ...
    if action == "setchannel":
        ...
        if not ch:
            ...
            return True
        # нормализация: если только username без @ — добавим @
        ...
        await bot.send_message(chat_id,
            ...
            f"⚠️ Убедитесь, что бот добавлен в этот канал как администратор.",
    ...
    if data.startswith("admin:"):
        ...
        if sub == "channel":
            ...
            txt = (
                ...
                f"{ch}\n\n"
                f"Если канал задан — при получении карты бот проверит подписку "
                f"и выдаст +1 карту (один раз).\n\n"
        ...
        if sub == "setchannel":
            ...
            await bot.edit_message_text(chat_id, message_id,
                "📡 Отправьте @username канала или его ID (-100...).\n\n"
                "Пример: @YourPrince\n\n/cancel — отмена.",
                reply_markup=cancel_kb())
            await bot.answer_callback(cq_id)
            return
        ...
        if sub == "clearchannel":
            ...
            store.save()
            await bot.edit_message_text(chat_id, message_id,
                "✅ Канал подписки убран. Бонус за подписку отключён.",
                reply_markup=channel_kb())
            await bot.answer_callback(cq_id)
            return
