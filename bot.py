...
ALTGRAM_DOMAIN = "https://altgram.xyz"
...
# Если True — при ошибке проверки подписки доступ закрывается.
# Если False — при ошибке (не при явном "не подписан") доступ разрешается.
...
CAT_BREEDS = [
    ...
    "Сфинкс","Британец","Перс","Мейнкун","Бенгал","Саванна","Сибиряк","Скоттиш","Донской","Бархан",
    "Иней","Лёд","Снег","Град","Дождь","Гроза","Радуга","Заря","Закат","Рассвет",
    "Полдень","Полночь","Звезда","Комета","Метеор","Астероид","Галактика","Туманность","Квазар","Алый",
    "Багровый","Бирюзовый","Васильковый","Голубой","Жёлтый","Зелёный","Золотой","Изумрудный","Коралловый","Лавандовый",
    "Лазурный","Лимонный","Малиновый","Медный","Мятный","Небесный","Огненный","Оливковый","Оранжевый","Пурпурный",
    "Розовый","Рубиновый","Салатовый","Сапфировый","Серебряный","Сиреневый","Фиолетовый","Хрустальный","Янтарный","Арчи",
    "Барон","Вольт","Гоша","Дэн","Ерошка","Жулик","Зоркий","Искра","Клёпа","Лютик",
    "Малыш","Нильс","Огонёк","Пегас","Рысь","Соболь","Тимка","Умник","Фантик","Хитрец",
    "Цыган","Чип","Электрон","Юла","Яхонт","Агат","Берилл","Гранит","Доломит","Жемчуг",
    "Кварц","Лазурит","Малахит","Нефрит","Опал","Рубин","Топаз","Янтарь","Ассоль","Бриз",
    "Вьюга","Дюна","Ель","Жара","Зима","Капель","Лето","Метель","Ночь","Осень",
    "Пурга","Роса","Снежинка","Туман","Утро","Весна","Эхо","Юг","Ясень","Арфа",
]
...
CAT_SUFFIX = [
    ...
    "Сказочный","Легендарный","Мифический","Бессмертный","Вечный","Юный","Старый","Новый","Забытый","Известный",
    "Тайный","Явный","Скрытый","Открытый","Закрытый","Светлый","Тёмный","Яркий","Тусклый","Блестящий",
    "Матовый","Прозрачный","Плотный","Лёгкий","Тяжёлый","Горячий","Холодный","Сухой","Мокрый","Сладкий",
    "Кислый","Солёный","Горький","Пряный","Свежий","Старинный","Модный","Стильный","Элегантный","Сложный",
    "Храбрый","Трусливый","Добрый","Злой","Щедрый","Жадный","Честный","Хитроумный","Бывалый","Дерзкий",
]
    ...
    async def call(self, method: str, **params):
        ...
        try:
            async with self.session.post(url, json=params) as r:
                ...
                if not data.get("ok"):
                    log.warning(f"API error {method} {params}: {data}")
                return data
...
def channel_url(ch: str) -> Optional[str]:
    """AltGram-ссылка на канал. Для @username — altgram.xyz/<username>."""
    if not ch:
        return None
    if ch.lstrip("-").isdigit():
        return None
    return f"{ALTGRAM_DOMAIN}/{ch.lstrip('@')}"
...
async def _try_get_member(ch_id, user_id) -> Optional[bool]:
    ...
    r = await bot.get_chat_member(ch_id, user_id)
    if not r.get("ok"):
        log.info(f"getChatMember('{ch_id}', {user_id}) failed: {r.get('description')}")
        return None
    try:
        status = r["result"].get("status", "")
    except Exception:
        return None
    log.info(f"getChatMember('{ch_id}', {user_id}) -> status={status}")
    if status in ("creator", "administrator", "member"):
        return True
    if status in ("left", "kicked", "restricted"):
        # restricted может быть True — но проверим is_member
        is_member = r["result"].get("is_member")
...
async def check_subscription(user_id: int) -> bool:
    ...
    # 3) numeric id из @channel (если API умеет resolve)
    r = await bot.get_chat(ch)
    log.info(f"getChat('{ch}') -> {r}")
    if r.get("ok"):
        cid = r["result"].get("id")
        if cid:
            res = await _try_get_member(cid, user_id)
            if res is not None:
                return res

    # 4) Не удалось проверить — зависит от STRICT_SUBSCRIPTION
...
async def handle_callback(cq: dict):
    ...
    if data == "sub:check":
        ...
        if ok:
            ...
            await bot.answer_callback(cq_id, "❌ Подписка не найдена. Проверьте, что бот админ канала.", show_alert=True)
        return
    ...
    if data.startswith("admin:"):
        ...
        if sub == "channel":
            ...
            txt = (
                ...
                f"⚠️ Бот должен быть администратором канала.\n"
                f"Проверка: {'жёсткая' if STRICT_SUBSCRIPTION else 'мягкая'}"
            )
