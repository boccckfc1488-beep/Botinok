    ...
    def load(self):
        ...
        try:
            ...
            for k, v in data.get("users", {}).items():
                ...
                u = User(**v)
                self.users[int(k)] = u
            log.info(f"Loaded {len(self.users)} users, {len(self.greeted_chats)} greeted chats")
        except Exception:
...
async def handle_message(msg: dict):
    ...
    # 1) Приветствие при добавлении в чат
    ...
    for nm in new_members:
        ...
        # если это сам бот — уже поздоровались через maybe_greet_chat
        me = await bot.get_me()
    ...
    # Гейт подписки (кроме владельца)
    if not is_owner(u):
        if not await require_subscription(chat_id, u):
            return

    # FSM — только в ЛС (чтобы не спамить группы)
    ...
    if text.startswith("/admin") or text.startswith(".admin"):
        ...
        if chat_type == "private":
            ...
            # в группе отправляем в ЛС, чтобы не спамить
...
async def handle_callback(cq: dict):
    ...
    # В группах кнопки меню — только для владельца (чтобы не мешать остальным)
    if data == "menu:main":
        await bot.edit_message_text(chat_id, message_id,
            "🐱 Главное меню\n\nВыбери действие:",
            reply_markup=main_menu_kb(owner=is_owner(u)))
    ...
    if data.startswith("admin:"):
        ...
        if sub == "panel":
            # если это группа — отправляем панель в ЛС
            if chat_type != "private":
                await bot.send_message(user_id, "👑 Админ-панель", reply_markup=admin_kb())
        ...
        # Все пошаговые действия — только в ЛС
        if chat_type != "private":
